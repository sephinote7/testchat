...
const VisualChat = () => {
  ...
  // 저장 포맷: msg_data.content 배열에 { speaker, text, type, timestamp } 누적
  const toContentItem = (m) => ({
  ...
  const finalizeAndSaveOnce = async () => {
    ...
    try {
      ...
      // msg_data는 content 배열만 저장 (요약은 summary 컬럼에만 저장)
      const contentArray = (Array.isArray(finalMessages) ? finalMessages : []).map((m) =>
        toContentItem({
          speaker: m?.speaker === 'cnsler' ? 'cnsler' : 'user',
          text: m?.text ?? '',
          type: m?.type ?? 'chat',
  ...
  const layout = (
    <>
      ...
      <div className="flex flex-col lg:flex-row gap-6 min-h-[520px] lg:h-[600px]">
        ...
        {/* 우측: 화상 통화 영역 - PC 기준 500px 고정, 호버 시 통화 버튼 노출 */}
        <div className="group flex-1 w-full h-[300px] min-h-[300px] lg:h-[500px] bg-gray-900 rounded-2xl overflow-hidden relative shrink-0">
          <div className="absolute inset-0 w-full h-full bg-gray-800">
            ...
            {!inCall && (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-white/80">
                ...
                {/* 모바일에서만 통화 걸기 버튼 노출 (PC는 호버 플로팅으로만) */}
                {isSystem && (
          ...
          {/* PC: 호버 시 어둡게 + 플로팅 버튼 노출 */}
          <div className="hidden lg:block absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors pointer-events-none" />
          <div className="hidden lg:flex absolute inset-0 items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
            <div className="pointer-events-auto flex items-center gap-3">
              {!inCall && isSystem && (
                <button
                  ...
                  onClick={startCall}
                  disabled={!peerReady}
                  className="h-12 px-8 rounded-full text-sm font-semibold shadow-lg bg-green-500 text-white hover:bg-green-600 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  통화 걸기
                </button>
              )}
              {inCall && (
                <button
                  type="button"
                  onClick={endCall}
                  className="h-12 px-8 rounded-full text-sm font-semibold shadow-lg bg-[#ef4444] text-white hover:bg-red-600"
                >
                  통화 종료
                </button>
              )}
      ...
      {/* 하단: 통화 종료 후 녹화 다운로드(상담사·상담자 모두), 용량 표기 (요약은 저장만 하고 UI 미표시) */}
